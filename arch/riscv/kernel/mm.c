...
void buddy_init() {
    ...
    if (!IS_POWER_OF_2(buddy_size)) buddy_size = fixsize(buddy_size);
...
void page_ref_inc(uint64_t pfn) { buddy.ref_cnt[pfn]++; }
...
void page_ref_dec(uint64_t pfn) {
    ...
    if (buddy.ref_cnt[pfn] == 0) {
        Log("[S] " YELLOW "CoW: " BLUE "free page because of no references: %p", PFN2PHYS(pfn));
        buddy_free(pfn);
    }
}

void buddy_free(uint64_t pfn) {
    if (buddy.ref_cnt[pfn]) return;  // Still in use
...
uint64_t buddy_alloc(uint64_t nrpages) {
    ...
    if (buddy.bitmap[index] < nrpages) return 0;
    ...
    while (index) {
        index               = PARENT(index);
        buddy.bitmap[index] = MAX(buddy.bitmap[LEFT_LEAF(index)], buddy.bitmap[RIGHT_LEAF(index)]);
    }
